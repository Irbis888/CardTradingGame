...
var CardScene = load(GamePaths.SCENE_CARD_BUTTON)
...
func gen_summary():
	...
	var out = Globals.content.ui.summary_title + "\n\t\t"
	if cost < 0:
		out += Globals.content.ui.lose_money_prefix + str(-cost)
	elif cost > 0:
		out += Globals.content.ui.get_money_prefix + str(cost)
	out += "\n" + Globals.content.ui.give_header + "\n"
	for i in give:
		out += player.collection[i].name + "\n"
	out += "\n" + Globals.content.ui.take_header + "\n"
	for i in take:
...
func redraw_ui():
	...
	player_money_label.text = Globals.content.ui.money_prefix + str(player.LI)
	trader_money_label.text = Globals.content.ui.money_prefix + str(counter.LI)
	...
	if counter.rank == Globals.CardRanks.ACE:
		trader_frame.texture = load(GamePaths.ACE_FRAME)
	else:
		trader_frame.texture = load(GamePaths.CHARACTER_FRAME)
	
	if player.rank == Globals.CardRanks.ACE:
		player_frame.texture = load(GamePaths.ACE_FRAME)
	else:
		player_frame.texture = load(GamePaths.CHARACTER_FRAME)
	...
	for i in player.collection:
		...
		day_counter.text = Globals.content.ui.day_prefix + str(Globals.day)
